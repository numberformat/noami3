...
translate( [ -61.5 , 3.2 , -1.2 ] ) cube( [ 98.5 , 41.2 , 10 ] );  // LCD window NV: made window larger
...
module pcb_clip(){
    difference(){
    union(){ // NV: improved the pcb clip design
    translate( [ -3 , -12 , 16.5 ] ) cube( [ 7 , 4 , 6 ] ); 
    //translate( [ -3 , -10.6 , 12.5 ] ) cube( [ 1 , 2.6 , 7 ] );  
    //translate( [ 3 , -10.6 , 12.5 ] ) cube( [ 1 , 2.6 , 7 ] );  
        }
    translate( [ -4 , -7 , 18.5 ] ) rotate([45,0,0]) cube( [ 10 , 6 , 6 ] );  
    
    translate( [ -4 , -7 , 11.5 ] ) rotate([45,0,0]) cube( [ 10 , 6 , 6 ] );  
...
main_body();
// reset button
translate( [ 62.5 , 37.3 , 0 ] ) cylinder( h = 8.2, r = 3.5, $fn=30); // NV: made a little taller
...
// SD card opening
translate( [ -80 , 9 , 14.5 ] ) cube( [ 10 , 28 , 15 ] );  
...
translate([50,52,0.6]) rotate([180,0,0]) linear_extrude(height = 2) 
{ text("NOAMi3",font = "helvetica:style=Bold", size=4, center=true); }
...
// LCD corners
//translate( [ -61.4 , 3.5 , -5.2 ] ) rotate([45,0,0]) cube( [ 98.4 , 5 , 5 ] );  // LCD window NV
//translate( [ -61.4 , 44 , -5.2 ] ) rotate([45,0,0]) cube( [ 98.4 , 5 , 5 ] );  // LCD window NV
