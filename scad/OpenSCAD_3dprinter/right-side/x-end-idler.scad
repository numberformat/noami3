...
module x_end_idler_holes()
{
    ...
    translate([0,3.5,0])
    {
      ...
      rotate([0,0,30])cylinder( h = 4, r = nut_trap_radius, $fn=6);  // NV: increase size so nut actually fits
     }
        
    translate(v=[-24,-19,30.25]) rotate([0,-90,0]){
      rotate([0,0,30])cylinder( h = 4, r1 = 3.2, r2 = 7,  $fn=6);  
     }
...
module waste_pocket()
{
    ...
    // NV added hole for z axis end-stop adjustment  
    mirror([0,1,0])translate([-21,5,40]) cylinder(30, d=3, $fn=25);  
...
module x_end_idler()
{
    ...
    difference()
    {
        ...
        { text("R3",font = "helvetica:style=Bold", size=4); }   
    }
