...
// NV: Modified for T8 Lead Screw 8mm Acme

use <bearing.scad>
use <polyholes.scad>
//use <../../OpenSCAD_3dprinter_parts/lead-screw/lead-screw-T8.scad>

rod_distance = 45;
nut_trap_radius=3.5; // NV
rod_diameter=8; // NV
...
// Bottom pushfit rod
    translate(v=[-15,-41,6]) rotate(a=[-90,0,0]) pushfit_rod(rod_diameter,50); //NV

// Top pushfit rod
    translate(v=[-15,-41.5,rod_distance+6]) rotate(a=[-90,0,0]) pushfit_rod(rod_diameter,50); //NV

// TR Nut trap
   // Hole for the nut
    //#translate(v=[0,-17, -1]) poly_cylinder(h = 9.01, r = 6.7, $fn = 60);
    translate(v=[0,-17, -1]) cylinder(h = 14.51, r = 5.4, $fn = 60);
    translate(v=[0,-17, -0.1]) cylinder(h = 1, r1 = 5.5,r2 = 5, $fn = 60); // NV: 

// Screw holes for TR nut
   
   
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 8, -4]) cylinder(h = 19, r = 1.9, $fn=50);
    
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -8, -4]) cylinder(h = 19, r = 1.9, $fn=50);
        
    
  
    translate(v=[-1,-16,0]) rotate([0,0,-135]) translate([0,9.5,-1]) cylinder(h=2, r1=2.2,r2=1.65, $fn=50);
    translate(v=[1,-18,0]) rotate([0,0,-135]) translate([0,-9.5,-1]) cylinder(h=2, r1=2.2,r2=1.65,, $fn=50);


// Nut traps for TR nut screws
// NV: extra space added for the nut traps
    translate(v=[-1,-16, 0]) rotate([0, 0, -135]) translate([0, 9.5, 11]) rotate([0, 0, 0])cylinder(h = 6, r = nut_trap_radius, $fn=6);    
    translate(v=[1,-18, 0]) rotate([0,0,-135]) translate([0,-10,10]) rotate([0,0,30])cylinder(h = 4, r = nut_trap_radius, $fn=6); 
    translate([-4.8,-18.2,10]) rotate([0,0,32]) cube([5,5.9,3.7]);
    translate([.2,-17.2,10]) rotate([0,0,58]) cube([5,10,3.7]);
...
module x_end_plain()
{
    ...
    //translate(v=[0,-17, -10]) rotate([0,0,45]) leadscrew_T8();
