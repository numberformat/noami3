...
use <../right-side/x-end.scad>

module x_end_motor_base()
{
    x_end_base();
    translate(v=[-15,31,26.5]) cube(size = [17,44,53], center = true);
    x_end_motor_endstop_base();		// NV: custom limit switch endstop base
}
...
module x_end_motor_endstop_holes()
{
    translate([-23.5,-28.5,58]){
        translate([17/2,7.5,-3]){
            ...
            translate([-4.75,0,0])cylinder(d=1.9,h=19,$fn=20);
            translate([-4.75,0,6])cylinder(r1=1, r2=1.5, h=2,$fn=20); // NV: keep this improvement
...
/**
 * Screw hole for left z endstop adjustment screw
 */
module x_end_motor_zlimit_screw_hole()
{
    translate([-16,-6,51.2]) {       
        translate([-4.75,0,-11])cylinder(d=3,h=29,$fn=20);
        translate([-4.75,0,6])cylinder(r1=1.5, r2=3.5, h=2,$fn=20); 
    }            
}
...
module x_end_motor_holes()
{
 ...
 // Position to place
  ...
  // Material saving cutout
        ...
        translate([-35+2.8,10,-25]) rotate([45,0,0])  cube(size = [20,2.55,5.8]); // NV: increased size
        
        
 }
 x_end_motor_zlimit_screw_hole();
}
 ...
 difference()
        {
            ...
            x_end_motor_holes();
            x_end_motor_endstop_holes();      // NV: holes for custom limit switch mount
      ...
      translate([-15,3.5,51]) rotate([90,0,0]) cylinder(h=3, r1=5, r2=4, $fn=30); 
            ...
            // translate([-30,-30,58]) cube([30,30,10]); // NV: commented out since it was interfering with limit switch base.
            
            // version
            translate([-23.2,-20,2]) rotate([90,0,270]) linear_extrude(height = 0.6) 
            { text("R3",font = "helvetica:style=Bold", size=4, center=true); }   
            
    translate([-47,-10.3,60]) rotate([0,45,0]) cube([20,80,20]);    
    translate([-25,-40,49]) rotate([45,0,0]) cube([30,20,20]);   // NV: reduce chamfer so switch has more support
