...
use <../OpenSCAD_electronics/printer_components/mks_gen_1_4.scad>
...
if(displayBoard) {
  translate ([66.9,51.4,10]) rotate(90) 
    %import("lib/SKR_1.3_-_Low_Fi.stl");
}
...
module main_body()
{
    ...
    // rear panel 
    translate( [0,depth-2,0]) cube([width-14.2,2,height]);  
    ...
    translate([2,depth-7,0]) cube([5.5,7,height]); 
    ...
    translate([width-16,14,1]) rotate([0,0,0]) cylinder( h = 7,r = 5.5,$fn=6); 
    ...
    // door closing corner
    translate([width-18,depth-3.5,35]) cylinder( h = 3,r1=2.5,r2=1,$fn=30);   
...
module cutouts(){
    ...
    // USB connector hole
    translate([width-35.2,depth-4,7]) rotate ([0,0,90]) cube([5.5,14,14]);   // todo

    // SD card hole
    translate([width-52.2,depth-4,7]) rotate ([0,0,90]) cube([5.5,18,14]);   // todo
    ...
    // inner edges cutout
    //#translate([6,depth-12,7]) rotate([0,0,45]) cube([5,5,34] ); 
...
module skr_1_3_base()
{
    difference()
    {
            ...
            {text("NOAMi3  SKR V1.3",font = "helvetica:style=Bold",size=6,center=true); }    
